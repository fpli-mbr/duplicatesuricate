...
source=pd.read_csv(path_sample+'f40_data.csv',index_col=0,nrows=1000)
print(source.shape[0])
target=pd.read_csv(path_sample+'p11_data.csv',index_col=0)
...
irl = RecordLinker(n_estimators=50)
irl.train()
...
results.to_csv(path_sample+'results.csv',sep=',',encoding='utf-8')
print(pd.datetime.now())