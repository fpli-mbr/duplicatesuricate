...
def token_score_spark(df, left, right, outputCol='token_score'):
    ...
    pattern = "[-.;,| /]+"
    transform_left_token = RegexTokenizer(inputCol=left, outputCol=tk_left, pattern=pattern)
    transform_right_token =RegexTokenizer(inputCol=right, outputCol=tk_right, pattern=pattern)
    ...
    def transform_ratio(df, outputCol=outputCol):
        ...
        df = df.withColumn(outputCol, F.greatest('ratio1', 'ratio2', 'ratio3'))
        return df
